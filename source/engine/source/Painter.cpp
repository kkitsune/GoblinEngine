...
Painter::Painter(Painter&& other) : _vg(other._vg)
{
	other._vg = nullptr;
}
...
{
	auto p = Core::instance().settings().texture_path() / file;
	if(!boost::filesystem::exists(p)) throw std::invalid_argument("Image file '" + p.generic_string() + "' does not exist");
	return nvgCreateImage(_vg, p.string().c_str(), static_cast<int>(flags));
}
...
{
	auto p = Core::instance().settings().font_path() / file;
	if(!boost::filesystem::exists(p)) throw std::invalid_argument("Font file '" + p.generic_string() + "' does not exist");
	return nvgCreateFont(_vg, name.c_str(), p.string().c_str());
}
